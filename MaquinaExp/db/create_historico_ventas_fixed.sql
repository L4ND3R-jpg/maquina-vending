-- Script: create_historico_ventas_fixed.sql
-- Crea la secuencia, la tabla HISTORICO_VENTAS, índices y trigger (autónomo)
...
CREATE TABLE HISTORICO_VENTAS (
  ...
  ID_PRODUCTO        NUMBER,
  ID_MAQUINA         NUMBER,
  C_PRODUCTO         VARCHAR2(200),
  D_DESCRIPCION      VARCHAR2(1000),
  CANTIDAD           NUMBER(10) NOT NULL,
  PRECIO_UNITARIO    NUMBER(14,4),
  TOTAL              NUMBER(18,4),
...
/
-- 4) Trigger autónomo que registra ventas en HISTORICO_VENTAS
...
BEGIN
  ...
  -- Tomar identificadores desde la fila nueva si existen
  BEGIN
    v_prod_id := :NEW.PROD_ID_PRODUCTO;
  EXCEPTION WHEN OTHERS THEN v_prod_id := NULL; END;
  BEGIN
    v_maq_id := :NEW.MAQ_ID_MAQUINA;
  EXCEPTION WHEN OTHERS THEN v_maq_id := NULL; END;
...
-- 5) Índice combinado adicional para reportes por fecha+producto
